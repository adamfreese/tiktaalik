...
module matrices_wilson
  ...
  real(dp), allocatable, dimension(:,:) :: re_Cq0_dvcs, im_Cq0_dvcs, &
      ...
      & re_Ctilq0_dvcs, im_Ctilq0_dvcs, &
      & re_Ctilq1_dvcs, im_Ctilq1_dvcs, re_CtilG1_dvcs, im_CtilG1_dvcs

  public :: make_wilson_matrices, &
      & Cq_dvcs, CG_dvcs, Ctilq_dvcs, CtilG_dvcs
  ...
  contains
    ...
    subroutine make_wilson_matrices(nQ2)
        ...
        call make_dvcs_Ctilq0(nx, nxi, nQ2, Q2, grid_type)
        call make_dvcs_Ctilq1(nx, nxi, nQ2, Q2, grid_type)
        call make_dvcs_CtilG1(nx, nxi, nQ2, Q2, grid_type)
    ...
    function Ctilq_dvcs(nxi, nx, nQ2, l_nlo) result(M)
        integer,  intent(in) :: nxi, nx, nQ2
        logical,  intent(in) :: l_nlo
        complex(dp), dimension(nxi, nx, nQ2) :: M
        complex(dp), dimension(nxi, nx) :: M0, M1
        ...
        integer  :: iq
        Q2 = get_Q2(nQ2)
        M0 = re_Ctilq0_dvcs + i_*im_Ctilq0_dvcs
        M1 = re_Ctilq1_dvcs + i_*im_Ctilq1_dvcs
        !$OMP PARALLEL DO
        do iq=1, nQ2, 1
          if(l_nlo) then
            M(:,:,iq) = M0 + get_alpha_QCD(Q2(iq))/(4.*pi)*M1
        ...
        !$OMP END PARALLEL DO
    end function Ctilq_dvcs

    function CtilG_dvcs(nxi, nx, nQ2, l_nlo) result(M)
        integer,  intent(in) :: nxi, nx, nQ2
        logical,  intent(in) :: l_nlo
        complex(dp), dimension(nxi, nx, nQ2) :: M
        complex(dp), dimension(nxi, nx) :: M0, M1
        ...
        Q2 = get_Q2(nQ2)
        ! For now, only LO
        M1 = re_CtilG1_dvcs + i_*im_CtilG1_dvcs
        if(l_nlo) then
          !$OMP PARALLEL DO
          do iq=1, nQ2, 1
            M(:,:,iq) = get_charges_squared(Q2(iq))*get_alpha_QCD(Q2(iq))/(4.*pi)*M1
    ...
    end function CtilG_dvcs

    ! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ! Private methods to make Wilson coefficient matrices: vector case
    ...
    ! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ! Private methods to make Wilson coefficient matrices: axial case

    subroutine make_dvcs_Ctilq0(nx, nxi, nQ2, Q2_array, grid_type)
        integer,  intent(in) :: nx, nxi, nQ2, grid_type
        real(dp), intent(in) :: Q2_array(nQ2)
        real(dp) :: xi(nxi)
        integer  :: ix, iz, iq, ic
        xi = get_xi(nxi)
        if(allocated(re_Ctilq0_dvcs)) deallocate(re_Ctilq0_dvcs)
        if(allocated(im_Ctilq0_dvcs)) deallocate(im_Ctilq0_dvcs)
        allocate(re_Ctilq0_dvcs(nxi,nx))
        allocate(im_Ctilq0_dvcs(nxi,nx))
        do iz=1, nxi, 1
        !$OMP PARALLEL DO
        do ix=1, nx, 1
          re_Ctilq0_dvcs(iz,ix) = pixel_coef(zero_func, re_Ctilq0_dvcs_sub, re_Ctilq0_dvcs_cst, xi(iz), nx, ix, grid_type)
          im_Ctilq0_dvcs(iz,ix) = pixel_coef(zero_func, zero_func,       im_Ctilq0_dvcs_cst, xi(iz), nx, ix, grid_type)
        end do
        !$OMP END PARALLEL DO
        end do
    end subroutine make_dvcs_Ctilq0

    subroutine make_dvcs_Ctilq1(nx, nxi, nQ2, Q2_array, grid_type)
        integer,  intent(in) :: nx, nxi, nQ2, grid_type
        real(dp), intent(in) :: Q2_array(nQ2)
        real(dp) :: xi(nxi)
        integer  :: ix, iz, iq, ic
        xi = get_xi(nxi)
        if(allocated(re_Ctilq1_dvcs)) deallocate(re_Ctilq1_dvcs)
        if(allocated(im_Ctilq1_dvcs)) deallocate(im_Ctilq1_dvcs)
        allocate(re_Ctilq1_dvcs(nxi,nx))
        allocate(im_Ctilq1_dvcs(nxi,nx))
        do iz=1, nxi, 1
        !$OMP PARALLEL DO
        do ix=1, nx, 1
          re_Ctilq1_dvcs(iz,ix) = pixel_coef(re_Ctilq1_dvcs_reg, re_Ctilq1_dvcs_sub, re_Ctilq1_dvcs_cst, xi(iz), nx, ix, grid_type)
          im_Ctilq1_dvcs(iz,ix) = pixel_coef(im_Ctilq1_dvcs_reg, im_Ctilq1_dvcs_sub, im_Ctilq1_dvcs_cst, xi(iz), nx, ix, grid_type)
        end do
        !$OMP END PARALLEL DO
        end do
    end subroutine make_dvcs_Ctilq1

    subroutine make_dvcs_CtilG1(nx, nxi, nQ2, Q2_array, grid_type)
        integer,  intent(in) :: nx, nxi, nQ2, grid_type
        real(dp), intent(in) :: Q2_array(nQ2)
        real(dp) :: xi(nxi)
        integer  :: ix, iz, iq, ic
        xi = get_xi(nxi)
        if(allocated(re_CtilG1_dvcs)) deallocate(re_CtilG1_dvcs)
        if(allocated(im_CtilG1_dvcs)) deallocate(im_CtilG1_dvcs)
        allocate(re_CtilG1_dvcs(nxi,nx))
        allocate(im_CtilG1_dvcs(nxi,nx))
        do iz=1, nxi, 1
        !$OMP PARALLEL DO
        do ix=1, nx, 1
          re_CtilG1_dvcs(iz,ix) = pixel_coef(re_CtilG1_dvcs_reg, re_CtilG1_dvcs_sub, re_CtilG1_dvcs_cst, xi(iz), nx, ix, grid_type)
          im_CtilG1_dvcs(iz,ix) = pixel_coef(im_CtilG1_dvcs_reg, im_CtilG1_dvcs_sub, im_CtilG1_dvcs_cst, xi(iz), nx, ix, grid_type)
        end do
        !$OMP END PARALLEL DO
        end do
    end subroutine make_dvcs_CtilG1
